...
Makes the Qobuz desktop app for macOS show up in **Now Playing**: Control Center, the lock screen and the menu bar get the current track, artist, album, artwork and progress, and their play/pause/next/previous buttons control Qobuz.
...
1. **`watcher.mjs`** runs in the background as a per-user LaunchAgent.
   - When you open Qobuz normally, the watcher restarts it once with `--remote-debugging-port=9333 --remote-debugging-address=127.0.0.1`.
...
2. **`bridge.js`** runs inside the Qobuz page.
   ...
   - Remote commands click Qobuz's own player buttons.
...
# What has it done? A healthy run logs: watcher started / relaunching Qobuz with debug port / bridge: installed
...
Use `tools/cdp.mjs` to explore the live page, fix `src/bridge.js`, then re-run `./install.sh`. Pull requests are welcome.
...
- Seeking from Control Center isn't supported. The scrubber shows progress but can't move it.
- Qobuz's own media-key shortcut and the media session may both react to a key press. If you see a double toggle, please open an issue.
